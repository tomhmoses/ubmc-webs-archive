...
var Element = new Class({
	...
	addEvent: function(action, fn){
		...
		if (this != window) Unload.functions.push(function(){
			if(el.removeEvent) el.removeEvent(action, fn);
			el[action+fn] = null;
		});
		return this;
	},
...
var Unload = {
	...
	unload: function(){
		...
		if(window.removeEvent) window.removeEvent('unload', window.removeFunction);
...
Fx.Accordion = new Class({
	...
	initialize: function(togglers, els, options){
		this.options = Object.extend({
			startIndex:0,
			duration:300,
			onComplete:Class.empty,
		...
		els.each(function(el, i){
			el.heightFX = new Fx.Height(el, {
				duration: self.options.duration,
				onComplete:function(){
					return function(){
						self.options.onComplete(i);
					}
				}(),
				transition:self.options.transition
			});

			el.fadeFX = new Fx.Opacity(el, {
				duration: self.options.duration
			});

			if(i != self.options.startIndex){
				el.heightFX.set(0);
				el.fadeFX.set(0);
			}
		});
		...
		togglers.each(function(el, i){
			el.onclick = function(){
				return function(){
					self.toggleFX(i);
					if(!self.options.independent){
						self.toggleFX(self.currentIndex);
					}
					self.currentIndex = i;
				}
			}();
	...
	toggleFX: function(i){
		this.els[i].heightFX.toggle();
		this.els[i].fadeFX.toggle();
	}
...
var Cookie = {
	...
	get: function(key){
		...
		return decodeURIComponent(myValue);
	},
...
var Sortables = new Class({
	...
	initialize: function(elements, options){
		...
		var trash = new Element('div').injectInside(this.options.injectInside || $(document.body));
		$A(elements).each(function(el, i){
			var copy = $(el).clone().setStyles({
				'position': 'absolute',
				'opacity': '0',
				'width': $(el).offsetWidth + 'px', /* fix for floated element widths */
				'display': 'none'
			}).injectInside(trash);
...
Fx.Width = Fx.Style.extend({
	...
	initialize: function(el, options){
		...
		if(!this.element) el.style.overflow = 'hidden';
        else this.element.style.overflow = 'hidden';
	},

	toggle: function(){
		if (this.element.offsetWidth > 0) return this.custom(this.element.offsetWidth, 0);
		else return this.custom(0, this.element.scrollWidth);
	},

	show: function(){
		return this.set(this.element.scrollWidth);
	}
